...
async function loadSettings(){try{const s=await fetch('/settings').then(r=>r.json());$('target').value=s.oscTarget;$('sendPort').value=s.oscSendPort;$('receivePort').value=s.oscReceivePort;$('oscStatus').textContent=`Device: ${s.deviceName} · ${s.ip}`}catch(e){$('oscStatus').textContent='Could not read settings: '+e.message}}
