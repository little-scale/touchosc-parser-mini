...
- OSC path partials using constants, `name`, `parent.name`, and child index.
...
Layouts designed at 368 × 448 use native display coordinates. Other canvas sizes are scaled proportionally and centered with letterboxing. Colors are composited against the document background and converted to RGB565 for the panel. TouchOSC-style inactive fills, active fills, grid lines, bracket outlines, rotary arcs, radar amplitude rings, and control cursors are reproduced with the display's available raster primitives.
